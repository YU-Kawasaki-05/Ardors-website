...
- [x] `/products` が表示される。
- [x] `/saas` にアクセスすると `/products` に redirect される。
- [x] Header / Footer の表記が JA `プロダクト` / EN `Products` になる。
- [x] トップページのコピーが `価値を生む開発` / `事業の次の一歩` の方向に更新されている。
- [x] トップページに `プロジェクト数 7` が表示されていない。
- [x] プロフィール表示名が JA `かわさき`, EN `Yu Kawasaki` になっている。
- [x] プロフィールに回答済みの経歴・価値観・技術スタックが反映されている。
- [x] サービスページから価格表示が削除されている。
- [x] サービスに `業務改善・効率化支援` が含まれている。
- [x] Stripe は `相談可能` 程度の表現になっている。
- [x] Products ページに marubo と構想中プロダクト1件が掲載されている。
- [x] marubo は `学習塾向けAIチャット・生徒管理システム` として説明されている。
- [x] marubo の GitHub repo へ直接リンクしていない。
- [x] 実績が初期4件（marubo / Ardors website / 製薬業界AI・ITコンサル / インフラ業界受け入れテスト自動化）に更新されている。
- [x] 実績に根拠のない数値成果が表示されていない。
- [x] EN ページに未翻訳の日本語や仮コピーが出ていない。
- [x] EN 未対応実績は英語サイトで非表示になっている。
- [x] `/sitemap.xml` に `/products` が含まれている。
- [x] `pnpm lint` / `pnpm typecheck` / `pnpm build` が通る。
